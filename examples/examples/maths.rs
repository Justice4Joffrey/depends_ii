...
use depends::{
    derives::Value, error::EarlyExit, DepRef, DepRef2, DepRef3, Dependencies2, Dependencies3,
    Dependency, DerivedNode, HashSetVisitor, InputNode, NodeState, Resolve, UpdateDerived,
    UpdateInput,
};
...
// A distinct way to transform a set of dependencies.
#[derive(Operation)]
pub struct Sum;
...
impl<A: NumberLike, B: NumberLike> UpdateDerived<DepRef2<'_, A, B>, Sum> for NumberValueI32 {
    fn update(&mut self, deps: DepRef2<'_, A, B>) -> Result<(), EarlyExit> {
        self.value = deps.a.value() + deps.b.value();
        Ok(())
    }
}
...
impl<A: NumberLike, B: NumberLike, C: NumberLike> UpdateDerived<DepRef3<'_, A, B, C>, Sum>
    for NumberValueI32
{
    fn update(&mut self, deps: DepRef3<'_, A, B, C>) -> Result<(), EarlyExit> {
        self.value = deps.a.value() + deps.b.value() + deps.c.value();
        Ok(())
    }
}
...
impl<A: NumberLike> UpdateDerived<DepRef<'_, A>, Square> for NumberValueI32 {
    fn update(&mut self, deps: DepRef<'_, A>) -> Result<(), EarlyExit> {
