...
use depends::{
    ...
    DepRef, DepRef2, DepRef3, Dependencies3, Dependency, DerivedNode, InputNode, Resolve, OneRef, TargetMut,
...
impl UpdateDerived<DepRef<'_, OneRef<'_, OpenOrders>>, CheckRiskLimit> for RiskLimit {
    fn update(&mut self, deps: DepRef<'_, OneRef<'_, OpenOrders>>) -> Result<(), EarlyExit> {
        let orders = deps.value.len();
...
impl UpdateDerived<DepRef<'_, OneRef<'_, OpenOrders>>, CalculateNextNumber> for ExpensiveCalculation {
    fn update(&mut self, _: DepRef<'_, OneRef<'_, OpenOrders>>) -> Result<(), EarlyExit> {
    ...
    > for DecisionNode
{
    fn update(
        ...
        self.value = Some(OpenOrdersOperation::Add(deps.c.next_number));
        Ok(())
    }
}
